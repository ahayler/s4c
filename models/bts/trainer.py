...
class BTSWrapper(nn.Module):
    ...
    def forward(self, data):
        ...
        with profiler.record_function("trainer_sample_segmentation-rays"):
            segmentation_rays, seg_rgb_gt, segmentation_gt, patch_to_image = sampler.sample(
                images_ip[:,:n_segs], poses[:, :n_segs], projs[:, :n_segs], segs[:, :n_segs], sample_segs=True)

        data["patch_to_image"] = patch_to_image
        data["fine"] = []
        data["coarse"] = []
        data["segmentation"] = []
